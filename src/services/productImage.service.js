...
export const findAll = async (paranoid = true) => {
  return await db.ProductImage.findAll({ paranoid });
};
...
export const restore = async (productId) => {
  const productImages = await db.ProductImage.findAll({ where: { productId }, paranoid: false });

  if (!productImages) throwError('No product image found with the given id', 404);

  const productImage = productImages[0];

  return await productImage.restore();
};
